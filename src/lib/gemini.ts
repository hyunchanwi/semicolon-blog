...
export async function generateBlogPost(topic: string, searchResults: SearchResult[]): Promise<BlogPostResult> {
  ...
  `;
    ...
    try {
        ...
        let en_title = '';
        let en_content = '';
        const enMatch = text.match(/<!--EN_CONTENT_START-->[\s\S]*?<article>([\s\S]*?)<\/article>[\s\S]*?<!--EN_CONTENT_END-->/);
        
        if (enMatch) {
            const enHtml = enMatch[1].trim();
            // Extract English title
            const enTitleMatch = enHtml.match(/<h2[^>]*>([^<]+)<\/h2>/);
            en_title = enTitleMatch ? decodeHtmlEntities(enTitleMatch[1].trim()) : koreanTitle;
            en_content = decodeHtmlEntities(enHtml);
            en_content = ensureHtml(en_content);
            
            // Remove English block from Korean content
            text = text.replace(/<!--EN_CONTENT_START-->[\s\S]*?<!--EN_CONTENT_END-->/, '').trim();
            
            console.log(`[Gemini] ✅ English content generated: ${en_title.slice(0, 40)}...`);
        } else {
            console.warn('[Gemini] ⚠️ No English content block found in AI response');
        }
        ...
        // ── Final DOM Sanitization ──
        // Remove empty <p></p> or <p><br></p> wrappers often generated by AI
        const sanitizeHtml = (html: string) => {
            return html.replace(/<p>(?:\s*|<br\s*\/?>)*<\/p>/gi, '').trim();
        };

        text = sanitizeHtml(text);
        if (en_content) en_content = sanitizeHtml(en_content);
