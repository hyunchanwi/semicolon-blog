...
async function generateFromVideo(video: YouTubeVideo): Promise<{ title: string; content: string; slug?: string }> {
    ...
    const prompt = `현재 연도는 **2026년**입니다. 당신은 전문 IT 기술 칼럼니스트로서 아래 영상 정보와 최신 웹 정보를 종합하여 깊이 있고 전문적인 블로그 포스팅을 작성해야 합니다.
${createVideoPrompt(video)}${externalContext}
...
5. **형식**: Markdown 문법(###, **, - 등)을 절대 사용하지 마세요. **오직 HTML 태그** (<h3>, <p>, <ul>, <ol>, <li>, <strong>, <table>)만 사용해야 합니다.
...
- **심층 분석 (Deep Dive)**: 3개 이상의 소제목(<h3>)으로 나누어, 단순 소개를 넘어선 **원리 분석, 장단점 비교, 경쟁사와의 차이, 시장의 반응** 등을 매우 상세히 서술할 것.
